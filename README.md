...
4. **Stopping the bridge:** Press **Ctrl+C** in the terminal to flush the night's data to itals.db and seal the session cleanly.
...
| inary_sensor.cu_sleep_node_01_presence | Binary Sensor | Bed / Room occupancy |
| inary_sensor.cu_sleep_node_01_apnea_event | Binary Sensor | Active apnea breathing flatline |
| inary_sensor.cu_sleep_node_01_fall_detected | Binary Sensor | Safety alert for detected falls |
...
* **ESP32-C6-DevKitC-1** (–10) — Recommended for **WiFi 6 (802.11ax)** with 242 subcarriers (5× higher spatial resolution).
* **ESP32-S3-DevKitC-1** (–8) — 52 subcarriers (WiFi 4 / 802.11n).
...
| --host | 127.0.0.1 | Bind address ( .0.0.0 for local network access) |
| --db | itals.db | Path to SQLite database file |
...
* Core WiFi CSI capture and edge signal processing built upon the **[RuView](https://github.com/ruvnet/RuView)** platform by uvnet.
